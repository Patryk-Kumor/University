...
;; Ewaluator tutaj ma postać samej procedury w języku interpretowanym (postać quote)

(define e1 '(cons (quote +) (cons 1 (cons 2 null))))
(define e2 '(cons (quote -) (cons 3 (cons 4 null))))
(define e3 '(cons (quote *) (cons 5 (cons 6 null))))
(define e4 '(cons (quote /) (cons 7 (cons 8 null))))
(define e-err '(cons (quote /) (cons 7 (cons 0 null))))

(define evaluator
  '(let (second (lambda (e) (car (cdr e))))
   (let (third (lambda (e) (car (cdr (cdr e)))))
     (lambda-rec (eval e)
              (if (pair? e)
                  (cond 
                    [(eq? (car e) (quote +)) (+ (eval (second e))
                                                (eval (third e)))]
                    [(eq? (car e) (quote -)) (- (eval (second e))
                                                (eval (third e)))]
                    [(eq? (car e) (quote *)) (* (eval (second e))
                    ...
                    [(eq? (car e) (quote /))
                     (let (m (eval (third e)))
                       (if (= m 0)
                           ;; (error "div by 0")
                           ;; Tu powinien być ERROR, ale przecież go nie ma w naszym języku
                           ;; błąd zostanie zwrócony dopiero przez interpter z wykładu
                           ;; dla wyrażenia w takiej postaci: 
                           (/ (eval (second e))
                              (eval (third e)))
                           ;; else
                           (/ (eval (second e))
                              (eval (third e)))))]
                    [true 0]) ;; niezgodność arg -> podobnie jak div-by-0
                  e))
              )))
...
;; Ewaluator tutaj ma postać parametryzowaną

(define (evaluator-with-param what)
   (list 'let (list 'to-eval what)
   '(let (second (lambda (e) (car (cdr e))))
    (let (third (lambda (e) (car (cdr (cdr e)))))
         ((lambda-rec (eval e)
              (if (pair? e)
...
(eval (evaluator-with-param e1))
(eval (evaluator-with-param e2))
(eval (evaluator-with-param e3))
(eval (evaluator-with-param e4))
;(eval (evaluator-with-param e-err)) -> div by 0


(eval (evaluator-with-param '(cons (quote +) (cons (cons (quote -) (cons 3 (cons 4 null))) (cons 2 null)))))




(eval (evaluator-with-param '(cons (quote +)
                                   (cons (cons (quote +) (cons 1 (cons 2 null)))
                                   (cons (cons (quote +) (cons 1 (cons 2 null))) null)))))
