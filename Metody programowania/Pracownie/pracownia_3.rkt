...
(define (solve-ax+by=1 a b)
  ...
  ;; Równanie diofantyczne ax + by = c ma rozwiązanie wtedy i tylko wtedy,
  ;; gdy NWD(a, b) jest dzielnikiem liczby c  =>  NWD(a, b) = 1.
  ;; W invert-modulo sprawdzamy czy (gcd a b) = 1, tu tego zaniecham.
  (define (ndw xi-2 xi-1 xi yi-2 yi-1 yi a b q r)
      ...
      (if (= r 0)
          ...
          (let ([a b]
                ...
                [xi  (- xi-2 (* q xi-1))]
                [yi  (- yi-2 (* q yi-1))])
            (ndw xi-1 xi xi yi-1 yi yi a b (quotient a b) (- a (* b (quotient a b))) ))))
  (if (> b a)
      (let ([reversed-pair (solve-ax+by=1 b a)])
        (cons (cdr reversed-pair) (car reversed-pair)))
      (let ([q (quotient a b)]
            [r (- a (* b (quotient a b)))])
            ;[xi-2  1] [xi-1  0] [xi  1]
            ;[yi-2  0] [yi-1  1] [yi (- (quotient a b) 1)])
        ;(ndw xi-2 xi-1 xi yi-2 yi-1 yi a b q r))))
        (ndw 1 0 1 0 1 (- (quotient a b) 1) a b q r))))
...
(define (RSA-unconvert-list intlist key)
  ;;; XXX: Write your code here
  ;; 
  ;; Szyfrowanie   - L'[i] = (L[i] - L'[i-1]) % n ) ^ e 
  ;; 
  ;; Deszyfrowanie -  L[i] = ((L'[i] ^ d % n) + L'[i-1]) % n 
  ;;
  (let ((n (key-modulus key)))
    (define (convert l prev)
      (if (null? l)
          '()
          (let ([x (modulo (+ prev
                              (RSA-transform (car l) key)) n)])
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ...
  (let ([coded-message (RSA-encrypt message recipient-public-key)])
    ;; Tworzenie pary ( zaszyfrowana-wiadomość . (car x) ),
    ;; gdzie x = (szyfr (jednoelementowa_lista (numer-hasz-szyfru))), klucz prywatny),
    ;; co samo w sobie jest też jednoelementową listą (chcemy tylko numer)
    (make-signed-message coded-message
                         (car (RSA-convert-list (list (compress coded-message))
                                                sender-private-key)))))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  ;;; XXX: Write your code here
  ;; Wyliczamy hasz szyfru (podpis1), oraz odszyfrowujemy podpis2,
  ;; jeśli są równe to pomyślnie zweryfikowano, więc można spokojnie odszyfrować wiadomość.
  (let ([calculated-signature (compress (message cyphertext))]
        [decrypted-signature (car (RSA-unconvert-list (list (signature cyphertext)) sender-public-key))])
    (if (= calculated-signature decrypted-signature)
